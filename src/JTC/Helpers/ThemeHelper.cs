...
{
    // "Фирменная 1" — original logo palette (pink → orange), white text.
    ...
    // "Фирменная 2" — cool blue-navy → lime-green (rgb(50,65,102) → rgb(122,179,23)).
    private static readonly Color Brand2Top    = Color.FromArgb(0xFF, 0x32, 0x41, 0x66);
    private static readonly Color Brand2Bottom = Color.FromArgb(0xFF, 0x7A, 0xB3, 0x17);
    ...
    private static bool IsBrand(AppTheme t) => t == AppTheme.Brand || t == AppTheme.Brand2;

    private static (Color Top, Color Bottom) BrandColors(AppTheme t) =>
        t == AppTheme.Brand2 ? (Brand2Top, Brand2Bottom) : (BrandTop, BrandBottom);

    // Primary-accent trio for the "Save / Delete / Add" AccentButton in ContentDialogs.
    // Brand uses the top pink (#E52E71 → hover lighter, press darker). Brand2 uses the
    // bottom lime as the accent (reads as a positive-action button on the cool gradient),
    // with hover/press generated by pushing it toward white / black respectively.
    private static (Color Base, Color Hover, Color Pressed) AccentColors(AppTheme t) => t switch
    {
        AppTheme.Brand2 => (
            Color.FromArgb(0xFF, 0x7A, 0xB3, 0x17),
            Color.FromArgb(0xFF, 0x93, 0xCE, 0x2E),
            Color.FromArgb(0xFF, 0x5C, 0x87, 0x11)),
        _ => (
            BrandTop,
            Color.FromArgb(0xFF, 0xFF, 0x4A, 0x8C),
            Color.FromArgb(0xFF, 0xB0, 0x1F, 0x55)),
    };

    private static LinearGradientBrush BrandGradient() => BrandGradient(CurrentTheme);

    private static LinearGradientBrush BrandGradient(AppTheme theme)
    {
        var (top, bottom) = BrandColors(theme);
    ...
    public static void ApplyToDialog(ContentDialog dialog, AppTheme theme)
    {
        ...
        var (accBase, accHover, accPress) = AccentColors(theme);
        var brandPrimary  = new SolidColorBrush(accBase);
        var brandPrimaryH = new SolidColorBrush(accHover);
        var brandPrimaryP = new SolidColorBrush(accPress);

        dialog.Background = BrandGradient(theme);
        ...
        dialog.Resources["ComboBoxDropDownBackground"]                   = BrandGradient(theme);
        dialog.Resources["ComboBoxDropDownBackgroundPointerOver"]        = BrandGradient(theme);
        dialog.Resources["ComboBoxDropDownBackgroundPointerPressed"]     = BrandGradient(theme);
    ...
    public static void Apply(FrameworkElement root, AppTheme theme)
    {
        ...
        switch (theme)
        {
            ...
            default: // Brand + Brand2
                panel.Background = BrandGradient(theme);
                root.RequestedTheme = ElementTheme.Dark;
                break;
        }
        ...
        if (IsBrand(theme))
            ApplyBrandMenuFlyoutBrushes(theme);
        else
    ...
    private static void ApplyBrandMenuFlyoutBrushes(AppTheme theme)
    {
        ...
        res["MenuFlyoutPresenterBackground"]       = BrandGradient(theme);
